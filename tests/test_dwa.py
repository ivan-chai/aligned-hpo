...
class TestDWAWeightFormula(TestCase):
    ...
    def test_compute_weights_dwa_formula(self):
        encoder, head1, head2 = _make_model()
        opt = _make_optimizer(encoder, head1, head2, temperature=2.0)
    ...
    def test_temperature_affects_weights(self):
        ...
        encoder1, head11, head21 = _make_model()
        opt_low_T = _make_optimizer(encoder1, head11, head21, temperature=0.5)
        opt_low_T._loss_history = [L0, L1]
        w_low = opt_low_T._compute_weights()

        encoder2, head12, head22 = _make_model()
        opt_high_T = _make_optimizer(encoder2, head12, head22, temperature=10.0)
