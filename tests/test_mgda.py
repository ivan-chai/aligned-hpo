...
from aligned_hpo import MGDAOptimizer
from aligned_hpo.aligned_hpo import HPO_STAGE_DOWNSTREAM
from aligned_hpo.mgda import _solve_min_norm
...
def _make_optimizer(encoder, head1, head2, head_down, n_tasks=2, **kwargs):
    ...
    return MGDAOptimizer(params, torch.optim.Adam, {"lr": 1e-3}, **kwargs)
...
class TestSolveMinNorm(TestCase):

    def test_uniform_gram_gives_uniform_alpha(self):
        # G = ones => all tasks equally conflicting => uniform weights
        G = torch.ones(3, 3)
        alpha = _solve_min_norm(G)
        self.assertEqual(alpha.shape, (3,))
        self.assertAlmostEqual(alpha.sum().item(), 1.0, places=5)
        self.assertTrue(alpha.ge(0).all())

    def test_identity_gram(self):
        # Orthogonal gradients => uniform weights (all grad norms equal)
        G = torch.eye(3)
        alpha = _solve_min_norm(G)
        self.assertAlmostEqual(alpha.sum().item(), 1.0, places=5)
        self.assertTrue(alpha.ge(0).all())

    def test_dominant_task_gets_zero_weight(self):
        # g0 and g1 point in opposite directions => one gets zero weight
        g0 = torch.tensor([1.0, 0.0])
        g1 = torch.tensor([-1.0, 0.0])
        G = torch.stack([g0, g1]) @ torch.stack([g0, g1]).T
        alpha = _solve_min_norm(G)
        self.assertAlmostEqual(alpha.sum().item(), 1.0, places=4)
        self.assertTrue(alpha.ge(-1e-6).all())

    def test_output_sums_to_one(self):
        torch.manual_seed(42)
        G = torch.randn(4, 8)
        G = G @ G.T + torch.eye(4) * 0.1
        alpha = _solve_min_norm(G)
        self.assertAlmostEqual(alpha.sum().item(), 1.0, places=5)
        self.assertTrue(alpha.ge(-1e-6).all())


class TestMGDAOptimizerConstruction(TestCase):
...
class TestMGDAOptimizerStep(TestCase):
    ...
    def test_step_runs(self):
        ...
        self.assertTrue(weights.ge(0).all(), "returned weights must be non-negative")

    def test_weights_sum_to_one(self):
        for _ in range(5):
            self._step()
        w_sum = self.optimizer.weights.sum().item()
        self.assertAlmostEqual(w_sum, 1.0, places=5)

    def test_weights_are_nonnegative(self):
        for _ in range(5):
            self._step()
        self.assertTrue(self.optimizer.weights.ge(-1e-6).all())

    def test_model_params_get_gradients(self):
    ...
    def test_wrong_closure_shape_raises(self):
        def bad_closure(down_weight, weights, retain_graph=False, stage=None):
            ...
            loss.backward()
            return torch.randn(3)  # wrong: should be (2,) — but MGDA discards return value

        # MGDA doesn't check losses shape (need_losses=False), so no error expected here.
        # The closure is called, grads are gathered; no shape check on return value.
        self.optimizer.hpo_step(bad_closure)  # should not raise

    def test_closure_returns_non_tensor_ok(self):
        # MGDA ignores the return value (no loss tracking), so non-tensor is fine
        def closure(down_weight, weights, retain_graph=False, stage=None):
            self.optimizer.zero_grad()
            z = self.encoder(self.x)
            loss = (weights * torch.stack([
                nn.functional.mse_loss(self.head1(z), self.y1),
                nn.functional.mse_loss(self.head2(z), self.y2),
            ])).sum() + down_weight * nn.functional.mse_loss(self.head_down(z), self.y_down)
            loss.backward(retain_graph=retain_graph)

        self.optimizer.hpo_step(closure)  # should not raise


class TestMGDAMetrics(TestCase):
...
class TestMGDAStateDict(TestCase):
...
class TestMGDAEncoderDecoder(TestCase):
    ...
    def test_weights_sum_to_one(self):
        ...
        self.assertAlmostEqual(opt.weights.sum().item(), 1.0, places=5)
...
class TestMGDAThreeTasks(TestCase):
    ...
    def test_three_tasks(self):
        ...
        opt = MGDAOptimizer(params, torch.optim.Adam, {"lr": 1e-3})
