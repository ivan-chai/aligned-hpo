...
class TestGradNormOptimizerConstruction(TestCase):
...
class TestGradNormOptimizerStep(TestCase):
    ...
    def test_weight_renormalization(self):
        for _ in range(5):
            self._step()
        w_abs_sum = self.optimizer.weights.abs().sum().item()
        self.assertAlmostEqual(w_abs_sum, 2.0, places=5)
    ...
    def test_initial_losses_set_on_first_step(self):
        ...
        self._step()
        self.assertIsNotNone(self.optimizer._initial_losses)
        self.assertEqual(self.optimizer._initial_losses.shape, (2,))
    ...
    def test_wrong_closure_shape_raises(self):
        def bad_closure(down_weight, weights, retain_graph=False, stage=None):
            ...
            loss.backward()
            return torch.randn(3)  # wrong: should be (2,)
...
class TestGradNormStateDict(TestCase):
    ...
    def test_state_dict_roundtrip(self):
        ...
        self.assertTrue(torch.allclose(opt2._initial_losses, opt1._initial_losses))
...
class TestGradNormThreeTasks(TestCase):
    ...
    def test_three_tasks(self):
        ...
        self.assertAlmostEqual(opt.weights.abs().sum().item(), 3.0, places=5)
        self.assertTrue(torch.isfinite(weights).all())
